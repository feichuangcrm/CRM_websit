...
.physics-blocks(
  ...
  client-only
    .icon-item.block-item(
      ...
      .bg-img(
        :style="{ backgroundImage: `url(${config.IMG_URL}/${logoUrl}.svg)` }"
      )
</template>
...
const initialization = (matter: typeof Matter) => {
  ...
  currentBodies =
    refs?.filter((ref) => getComputedStyle(ref).display === "block").length ||
    0;
...
const setupResize = () => {
  useResizeObserver(el, () => {
    ...
    if (
      prevCurrent !==
      (refs?.filter((ref) => getComputedStyle(ref).display === "block")
        .length || 0)
    ) {
      currentBodies =
        refs?.filter((ref) => getComputedStyle(ref).display === "block")
